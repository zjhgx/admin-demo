 ...
 *
 */
public class AcctItemAction extends BaseAction{
	private AcctItem acctItem;
	private InputStream jsonStream;
	private IAcctItemService acctItemService;
	private List list;
	private List belongTypeList = null;
	...
	public String list() throws Exception {
		belongTypeList = DictionaryUtil.getCodesByKey(AcctRecordConst.ACCT_ITEM_BELONG_TYPE);
		belongTypeList.remove(0);
		return "LIST";
	}
	...
	public String queryData() throws Exception {
		...
		if(!StringUtils.isEmpty(acctItem.getValueType())){
			cblist.add(new ConditionBean("valueType",acctItem.getValueType()));
		}
		if(StringUtils.isEmpty(acctItem.getBelongType())){
			cblist.add(new ConditionBean("belongType", ConditionBean.IN, new String[]{AcctItem.BelongType_Memeber, AcctItem.BelongType_MemberPoint}));
		}else{
			cblist.add(new ConditionBean("belongType",  acctItem.getBelongType()));
		}
	...
	public String toCreateAcctItem() throws Exception {
		belongTypeList = DictionaryUtil.getCodesByKey(AcctRecordConst.ACCT_ITEM_BELONG_TYPE);
		belongTypeList.remove(0);
		acctItem = new AcctItem();
		return "AddAcctItem";
	}
	
	public String createAcctItem() throws Exception {
		acctItem.setMiNo(SessionTool.getUserLogonInfo().getMiNo());
		
		{//判断编号重复
			ArrayList<ConditionBean> cblist = new ArrayList<ConditionBean>(4);			
			cblist.add(new ConditionBean("belongType", acctItem.getBelongType()));
			cblist.add(new ConditionBean("itemNo", acctItem.getItemNo()));
			cblist.add(new ConditionBean("miNo", acctItem.getMiNo()));
			if (AcctItem.BelongType_Memeber.equals(acctItem.getBelongType())){
				cblist.add(new ConditionBean("pointId", ConditionBean.IS_NULL ,null));
				
			}
			if (AcctItem.BelongType_MemberPoint.equals(acctItem.getBelongType())){
				cblist.add(new ConditionBean("pointId", acctItem.getPointId()));
			}
