...
$(function(){	
	...
	queryGrid=new DataPageComponent('contryData',{
		...
		frozenColumns:[[
	        		...
	        		singleSelect:false,
		url:'<s:url value="/demo/page_queryChooseMultiCountryList.jhtml"/>',
...
function doSelect(){	
	var rows = $('#contryData').datagrid('getSelections');	
	for(var i=0;i<rows.length;i++){
		//ids.push(rows[i][pkid]);
		addOption($("#"+$("#idDiv").val()),rows[i]["codeNo"],rows[i]["regionName"]);
	}
