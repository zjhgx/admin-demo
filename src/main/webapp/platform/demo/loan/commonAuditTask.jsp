...
<tiles:insertDefinition name="FUNC_TOOL_QUERY_DATA">
	...
	function initPage(){
		...
		dataGrid=new DataPageComponent("dataTable",{
			url:'<s:url value="/demo/loan_queryCommonAuditTask.jhtml"/>',
			columns:columns,
	...
	function dealTask(){
		var ids=getMutSelected("dataTable","taskId");
		if(ids.length<1){
			return;
		}		
		var url = "<s:url value='/platform/demo/loan/dealTask.jsp'/>?taskId="+ids;
		//alert(url);
		requestAtWindow(url,"add_edit",'Task Deal');
	}	
	
	function doTask(){
		var url = "<s:url value='/demo/loan_dealCommonAuditTask.jhtml'/>";
