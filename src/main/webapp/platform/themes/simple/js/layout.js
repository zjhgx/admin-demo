...
//		}
		...
		if(!!pp_refresh_timer){
			clearInterval(pp_refresh_timer);
		}
		...
		$("#jobs").tabs({
			"onSelect":function(title){
				if(title=="欢迎使用系统"){
					...
					if(typeof pp_refreshAllPortal != 'undefined'&&$.isFunction(pp_refreshAllPortal)){
						pp_refreshAllPortal();
						pp_refresh_timer=setInterval("pp_refreshAllPortal()",300000);
					}
...
function selectFirstJob(){
	$("#jobs").tabs('select','欢迎使用系统');
}
function taskDelegated(){
	var title="工作委托";
	var url = '/bpm/taskDelegated_main.jhtml';
	updateWorkapace(url,title);
}

function refreshWorkspace(basePath) {
	if ("undefined" != urlArray && null != urlArray && urlArray.length == 2) {
		history.go(-1);
		updateWorkapace(urlArray[0], titleArray[0]);
	} else {
