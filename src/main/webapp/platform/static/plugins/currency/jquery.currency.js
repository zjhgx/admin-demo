(function($) {
	function bindEvent(target){
		$(target).unbind('.currency');
		$(target).bind('keydown.currency', function(e){
			var ret = true;
			if (e.which == 45 || e.which == 46 || e.which==37 || e.which==39 || e.which==9){	//-
			}else if ((e.which >= 48 && e.which <= 57 && e.ctrlKey == false && e.shiftKey == false) || e.which == 0 || e.which == 8) {
			
			} else if (e.ctrlKey == true && (e.which == 99 || e.which == 118 || e.which==86 )) {
			...
			if ( e.which==37 || e.which==39 || e.which==9) {
				...
				$(target).currency("init");
			}
		}).bind('paste.currency', function(){
			var ret = true;
			...
			return ret;
		}).bind('dragenter.currency', function(){
			return false;
		});
	}
    $.fn.currency = function(method) {
        var methods = {
			...
			formatCurrency:function(value){
				var settings = $.extend({}, this.currency.defaults,{hidePrefix:true,hidePostfix:true,decimals:0});
				if ((""+value).indexOf(".") > -1) {
					settings.decimals=2;
				}
				return helpers.format_currency(value,settings);
			}
        };
...
function formatCurrency(value){
	return $("body").currency("formatCurrency",value);
}